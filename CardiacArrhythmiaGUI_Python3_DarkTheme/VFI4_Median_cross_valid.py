'''VFI4 INTRODUCTION OF POINT INTERVALS'''
import random
import math
from tkinter import *
...
Endpoints=[]
Classdist=[]
Ptintervals=[]      #Array to hold Point interval
Ptdist=[]           #Array to hold the distribution of class count of the point intervals
E=[]
k = 10
...
def countinterval():
        ...
        for i in range(class_total):
            for j in range (len(D[i])):
                for l in range((len(Endpoints[k])-1)):
                    if(D[i][j][k]!=999):
                        #Incrementing class count for Point Intervals
                        if(D[i][j][k] in Ptintervals[k]):
                            Ptdist1[str( D[i][j][k])][i]+=1
                        #Incrementing class count for range Intervals
    ...
    for k in range(features_no):
        ...
        s2.append(s1)
    #Normalisation of class count for the point intervals
    for j in range(features_no):
        for k in (Ptdist[j].keys()):
            for i in range(class_total):
                if(sum(Ptdist[j][k])!=0):
...
def find_interval(f,ef):
    #If feature value is a point interval
    if(ef in Ptintervals[f]):
        ...
        for k in range(class_total):
            if m<Ptdist[f][str(ef)][k] :
                ...
                classno = k
        return [classno,0,0]
    #If feature value lies in the range intervals                 
    else:        
