'''Program to run the KNN Algorithm using SKlearn'''
import numpy as np
import scipy
...
def knn(areAllAlgorithmExecuted):

    text_file = open("data_proper.txt", "r")
    child = Tk()
    child.title("KNN Accuracy")
    ...
    #To take in data in the form of a list from text file
    ...
    for offset in range(0,new_data,test_size):
        ...
        pred=(neigh.predict(K)) ##L[X:Y] selects the test data from x to y patients
